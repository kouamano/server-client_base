	...
	tmp_comm[0] = '\0';
        ...
        if(statement[0] == '\\'){
                ...
                if(strcmp(statement,"\\exit") == 0){
                        ...
                        printf("\\pscan <port>-<port>\t\tscan at port <port>.\n");
		...
		//UNDER CONSTRUCTION
                }else if(strncmp(statement,"\\pscan",6) == 0){
                        if(statement_len < 7){
                                return(-1);
                        }
                        sscanf(statement+6,"%d-%d",&firstport,&lastport);
                        printf("Port\tAlive\n");
			sprintf(tmp_comm,"%s","Alive?;");
			tmp_comm[strlen(tmp_comm)] = 0x04;
			tmp_comm[strlen(tmp_comm) + 1] = '\0';
                        if((firstport >= LOWERPORT)&&(lastport < LOWERPORT)){
                                printf("%d\t%s\n",firstport,client_nka(LOCALHOST,firstport,tmp_comm,1,0,1));
                        }else if((firstport >= LOWERPORT)&&(lastport >= LOWERPORT)){
                                for(i=firstport;i<=lastport;i++){
                                        printf("%d\t%s\n",i,client_nka(LOCALHOST,i,tmp_comm,1,0,1));
                                }
                        }else{
                                return(-1);
                        }
