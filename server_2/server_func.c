...
int ChildLoop(int acc, char *client_address, char *greeting){
	...
	int i;
	int ptr_command=0;
	int flag_end_command=0;

	/*allocation*/
	if((buf_rec=malloc(sizeof(char) * 8)) == NULL){perror("malloc\n");}
	if((buf_cp=malloc(sizeof(char) * LONG_BUFF_SIZE)) == NULL){perror("malloc\n");}
	if((buf_command=malloc(sizeof(char) * LONG_BUFF_SIZE)) == NULL){perror("malloc\n");}
	buf_rec[0]='\0';
	buf_cp[0]='\0';
	buf_command[0]='\0';
	...
	while(1){
		...
		buf_rec[len_rec]='\0';
		DB(fprintf(stderr,"buf_rec_size:%d:\n",len_rec));
		for(i=0;i<len_rec;i++){
			if(buf_rec[i] == ';'){
				buf_command[ptr_command+i] = '\0';
				flag_end_command=1;
			}else{
				buf_command[ptr_command+i] = buf_rec[i];
			}
		}
		if(flag_end_command == 1){
			len_command=strlen(buf_command);
			ptr_command=0;
			sprintf(buf_cp,"%s:[%s]\n",client_address,buf_command);
			send(acc,buf_cp,strlen(buf_cp),0); // send for client
			DB(fprintf(stderr,"PID=%d:%s",getpid(),buf_cp)); // output for server
			flag_end_command = 0;
			if(strcmp(buf_command,"end") == 0){
				break;
			}

		}else{
			ptr_command = ptr_command+i;
		}
		/*
		if(len_rec>0){
